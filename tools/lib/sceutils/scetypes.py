...
class MetadataInfo:
    ...
    def __str__(self):
        ret = ''
        ret += 'Metadata Info:\n'
        ret += ' Key:              {0}\n'.format(binascii.hexlify(self.key).decode())
        ret += ' IV:               {0}'.format(binascii.hexlify(self.iv).decode())
        return ret
...
class SceControlInfoDigest256:
    ...
    def __str__(self):
        ...
        ret += ' SCE Hash:         {0}\n'.format(binascii.hexlify(self.sce_hash).decode())
        ret += ' File Hash:        {0}\n'.format(binascii.hexlify(self.file_hash).decode())
...
class SceControlInfoDRM:
    ...
    def __str__(self):
        ...
        ret += ' Type Digest:       {0}\n'.format(binascii.hexlify(self.digest1).decode())
        ret += ' ECDSA224 Sig R:    {0}\n'.format(binascii.hexlify(self.sig2r).decode())
        ret += ' ECDSA224 Sig S:    {0}\n'.format(binascii.hexlify(self.sig2s).decode())
        return ret                   
...
class SceRIF:
    ...
    def __str__(self):
        ...
        ret += ' CID:               0x{0:X}\n'.format(self.cid)
        ret += ' Content ID:        {0}\n'.format(self.content_id)
        ret += ' KLicensee:         {0}\n'.format(binascii.hexlify(self.klicense).decode())
        ret += ' ECDSA160 Sig R:    {0}\n'.format(binascii.hexlify(self.sig1r).decode())
        ret += ' ECDSA160 Sig S:    {0}\n'.format(binascii.hexlify(self.sig1s).decode())
        return ret                           